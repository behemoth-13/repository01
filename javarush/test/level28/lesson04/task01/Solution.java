...
public class Solution {
    ...
    {
        private static volatile AtomicInteger factoriesCount = new AtomicInteger(0); // количество фабрик
        private volatile AtomicInteger factoryNumber = new AtomicInteger(0); // это номер конкретной фабрики
        private volatile AtomicInteger threadCount = new AtomicInteger(0); // это счетчик тредов

        public AmigoThreadFactory()
        {
            factoryNumber.set(factoriesCount.incrementAndGet());
        }
        ...
        public Thread newThread(Runnable r)
        {
            ...
            thread.setName(thread.getThreadGroup().getName() + "-pool-" + factoryNumber + "-thread-" + threadCount.incrementAndGet());
            return thread;
        }
    }
