...
public class Arcanoid
{
    ...
    public void draw(Canvas canvas)
    {
        ...
        brick.draw(canvas);//отрисуй кирпичи
    ...
    public void run() throws Exception
    {
        ...
        if (isWin)
            System.out.println("Лёха БОГ арканоида");
        else//Выводим сообщение "Game Over"
    ...
    public void checkBricksBump()
    {
        for (Brick brick : bricks)
        {
            if (ball.isIntersec(brick))//Тут проверь - столкнулся ли шарик с кирпичем.
            {
                bricks.remove(brick);
                double angel = Math.random() * 360;//Если да - кирпичь удалить, а шарик запустить в случайно направлении.
    ...
    {
        if (ball.isIntersec(stand))//Тут проверь - столкнулся ли шарик с подставкой.
        {
            double angel = 80 + Math.random()*20;
            ball.setDirection(angel);//Если да - запустить шарик  вверх на 80..100 градусов.
        }
    }
